  ...
  this.getDestinationOfPage = (index) => {
    const pager = {
      ...
      pageClusterPreviously: () => {
        ...
        const tags = (strings, ...values) => {
          let raw = ''
          const args = _.last(values)
          // eslint-disable-next-line no-magic-numbers
          const lastIndex = values.length - 1
          _.forEach(values, (val, idx, attr) => {
            if (idx < lastIndex) {
              raw = raw + strings[idx] + args[val]
            }
          })
          return raw
        }
        const hoge = { name: 'BRIDGE', DEVICE: 'eth0' }
        console.log(tags `BridgeCreate ${'name'} /DEVICE:${'DEVICE'}${hoge}`)
        this.isFirstSetupForThisCluster = false
        this.reloadFormDatas()
        return this.INDEX_OF_PAGE.pageTypeHw
      },
