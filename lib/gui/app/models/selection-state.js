...
/**
 * @summary Check if there is a Entered userdatas
 * @function
 * @public
 *
 * @returns {Boolean} whether there is a Entered userdatas
 *
 * @example
 * if (selectionState.hasUserdata()) {
 *   console.log('There is an userdatas!');
 * }
 */
exports.hasUserdata = () => {
  const uds = exports.getUserdata()
  if (_.isUndefined(uds)) {
    return false
  }
  return Boolean(constructionUD.getCurrentHostname(uds) !== '')
}
 ...
 * @public
 *
 * @param {{Object}} userdata - drive device
 *
 * @example
 * selectionState.setUserdata();
 */
exports.setUserdata = (userdata) => {
  store.dispatch({
    type: store.Actions.SET_USERDATA,
    data: userdata
  })
}

/**
 * @summary Get the userdata
 * @public
 *
 * @returns {Object} userdata
 *
 * @example
 * const userdata = selectionState.getUserdata();
 */
exports.getUserdata = () => {
  return _.get(store.getState().toJS(), [ 'selection', 'userdata' ])
}

/**
 * @summary Deselect userdata
 * @function
 * @public
 *
 * @example
 * selectionState.deselectUserdata();
 */
exports.deselectUserdata = () => {
  store.dispatch({
    type: store.Actions.DESELECT_USERDATA
  })
}
 ...
 * @example
 * const hostname = selectionState.getHostName();
 */
exports.getHostName = () => {
  const ud = _.get(store.getState().toJS(), [ 'selection', 'userdata' ])
  if (!_.isUndefined(ud)) {
    return ud.userDatas[ud.currentCluster].hostname
  }
  return ''
}

/**
 * @summary Get ClusterMode
 * @function
 * @public
 *
 * @returns {String} ClusterMode (SimpleMode|FullMode)
 * @example
 * const ClusterMode = selectionState.ClusterMode();
 */
exports.getClusterMode = () => {
  const ud = _.get(store.getState().toJS(), [ 'selection', 'userdata' ])
  if (!_.isUndefined(ud)) {
    const isSimple = ud.userDatas[ud.currentCluster].clusterData.isSimple
    if (isSimple) {
      return 'SimpleMode'
    }
    return 'FullMode'
