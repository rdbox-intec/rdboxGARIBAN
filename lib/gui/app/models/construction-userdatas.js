...
exports.dumpYamlFromCloudInit = (referrer) => {
  const raw = yaml.safeDump(referrer.userDatas[referrer.currentCluster].cloudInit)
  return raw
}
...
exports.setPasswordOfRoot = (referrer, rawPasswd) => {
  ...
  // eslint-disable-next-line no-magic-numbers
  referrer.userDatas[referrer.currentCluster].cloudInit.users[0].passwd = passwd
}
