...
module.exports = function (
  ...
  */
  ...
  this.addWpaSupplicantForYoursite = (ccode, ssid, passphrase = '', options = {}) => {
    this.userdata.addWpaSupplicantForYoursite(this.currentUserData, ccode, ssid, passphrase, options)
  }
  ...
  this.getWpaPassphrase = (plainText, ssid) => {
    if (!plainText || !ssid) {
      return plainText
    }
    // eslint-disable-next-line no-magic-numbers
  ...
  this.getDestinationOfPage = (index) => {
    const pager = {
      ...
      rdboxNetworkConnection: () => {
        const psk = this.getWpaPassphrase(this.formDatas.passphrase, this.formDatas.ssid)
        this.addWpaSupplicantForYoursite(this.formDatas.country,
          this.formDatas.ssid,
          psk)
