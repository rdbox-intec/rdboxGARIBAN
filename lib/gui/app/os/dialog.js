...
/**
 * @summary Open an Secret selection dialog
 * @function
 * @public
 *
 * @description
 * Notice that by Secret, etc files.
 *
 * @fulfil {Object} - selected Secret
 * @returns {Promise};
 *
 * @example
 * osDialog.selectSecret().then((keyFile) => {
 *   console.log('The selected keyFile is', image.path);
 * });
 */
exports.selectSecret = () => {
  return new Bluebird((resolve) => {
    electron.remote.dialog.showOpenDialog(currentWindow, {
