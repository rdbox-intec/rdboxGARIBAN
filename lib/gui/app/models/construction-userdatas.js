...
exports.vpncmdBuilder = (cmd) => {
  ...
  // Change For v0.1.7 Changed the VPN connection method from "bridge" to "client".
  switch (cmd.vpncmd) {
    case 'BridgeCreate':
      result = 'NicCreate rdbox'
      break
    case 'CascadeCreate':
      result = expandMaps `AccountCreate con_rdbox /SERVER:${'hostname'}:${'port'} /HUB:${'hubname'} /NICNAME:rdbox /USERNAME:${'username'}${cmd}`
      result += '\nAccountRetrySet con_rdbox /NUM:999 /INTERVAL:5'
      result += '\nAccountEncryptEnable con_rdbox'
      result += '\nAccountStartupSet con_rdbox'
      break
    case 'CascadePasswordSet':
      result = expandMaps `AccountPasswordSet con_rdbox /PASSWORD:${'password'} /TYPE:${'type'}${cmd}`
      result += '\nAccountConnect con_rdbox'
      result += '\nAccountStatusGet con_rdbox'
      break
    default:
      break
  }
...
exports.constructHostapd = (type) => {
  ...
  if (type === 'be') {
    ...
    base.type_content.interface = 'wlan4'
    base.type_content.hw_mode = 'g'
    base.type_content.channel = 1
    base.type_content.bridge = 'br0'
  } else {
    base.type_content.interface = 'wlan0'
  }
  return base
}
...
exports.constructVPNBridge = () => {
  // eslint-disable-next-line prefer-const
  let base = _.cloneDeep(UD_WRITE_FILE)
  base.path = '/usr/local/etc/vpnclient.in'
  base.type = 'vpnbridge'
  base.id = base.type
...
const evalWriteFile = {
  ...
  hostapd: (writeFileItem, isSimple) => {
    const hostapdId = writeFileItem.id
    let raw = ''
    _.forEach(writeFileItem.type_content, (value, key) => {
      if (isSimple) {
        if (value === '') {
        ...
        if (value !== '') {
          if (key === 'interface') {
            if (hostapdId === 'hostapd_be') {
              raw = `${raw}${key}=wlan2\n`
              writeFileItem.type_content.interface = 'wlan2'
            } else if (hostapdId === 'hostapd_ap_an') {
              raw = `${raw}${key}=wlan3\n`
              writeFileItem.type_content.interface = 'wlan3'
            } else if (hostapdId === 'hostapd_ap_bg') {
              raw = `${raw}${key}=wlan4\n`
              writeFileItem.type_content.interface = 'wlan4'
            } else {
              raw = `${raw}${key}=${value}\n`
            }
          } else {
  ...
  vpnbridge: (writeFileItem, isSimple) => {
    const vpncmds = writeFileItem.type_content
    const rawcmds = []
    _.map(vpncmds, (cmd) => {
      const ret = this.vpncmdBuilder(cmd)
      if (ret !== '') {
        rawcmds.push(ret)
      }
    })
  ...
  http_proxy: (writeFileItem, isSimple) => {
    writeFileItem.content = `http_proxy=${String(writeFileItem.type_content.http_proxy)}\n`
    return writeFileItem
  },
  no_proxy: (writeFileItem, isSimple) => {
    writeFileItem.content = `no_proxy=${String(writeFileItem.type_content.no_proxy)}\n`
    return writeFileItem
  },
  net_rules: (writeFileItem, isSimple) => {
    let raw = ''

    // Change For v0.1.4 change Default NIC.
    _.forEach(writeFileItem.type_content, (nic, index, array) => {
      if (nic.match(/wlan10/)) {
        const migNic = _.replace(writeFileItem.type_content[index], 'wlan10', 'wlan0')
        raw = `${raw}${migNic}\n`
      } else {
        raw = `${raw}${nic}\n`
      }
    ...
    // Change For v0.1.7 upd net.rules for RPi4.
    const tmplateNetrules = this.constructNetRules()
    if (writeFileItem.type_content.length < tmplateNetrules.type_content.length) {
      // eslint-disable-next-line no-plusplus
      for (let index = writeFileItem.type_content.length; index < tmplateNetrules.type_content.length; index++) {
        if (index === tmplateNetrules.type_content.length - 1) {
          raw = `${raw}${tmplateNetrules.type_content[index]}`
        } else {
          raw = `${raw}${tmplateNetrules.type_content[index]}\n`
        }
      }
...
exports.dumpYamlFromWriteFileList = (referrer) => {
  ...
  const delIndex = []
  const is_simple = referrer.userDatas[referrer.currentCluster].clusterData.isSimple
  _.forEach(referrer.userDatas[referrer.currentCluster].rdbox.write_files, (writeFileItem, index, array) => {
    if (!_.isUndefined(writeFileItem)) {
      const item = _.pick(evalWriteFile[writeFileItem.type](writeFileItem, is_simple), _.keys({ content: null, path: null }))
      if (item.content !== '') {
        if (referrer.userDatas[referrer.currentCluster].clusterData.isSimple) {
          if (writeFileItem.id === 'wpa_supplicant_ap_an' || writeFileItem.id === 'hostapd_ap_an') {
            delIndex.push(writeFileItem.id)
          } else {
  ...
  _.remove(referrer.userDatas[referrer.currentCluster].rdbox.write_files, (writeFileItem) => {
    return writeFileItem.id === 'wpa_supplicant_ap_an' || writeFileItem.id === 'hostapd_ap_an'
  })
...
exports.getRdboxWriteFilesByFilesId = (referrer, filesId) => {
  // eslint-disable-next-line no-magic-numbers
  if (this.getIndexOfWriteFs(referrer, filesId) === -1) {
    return {}
  }
