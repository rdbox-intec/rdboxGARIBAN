    ...
    try {
      ...
      _.forEach(dests, (dest) => {
        ...
        try {
          ...
          const files = options.userData.currentFiles
          const requiredFiles = _.filter(files, (file) => file.id === 'publicKey' || file.id === 'secretKey')
          ...
          if (process.platform === 'darwin') {
            ...
            const toDir = _.split(dest.drive.displayName, ',')[0]
            fs.writeFileSync(path.join(toDir, 'user-data'), userDataYml)
            _.forEach(requiredFiles, (file) => {
              if (file.from_path === '') {
                return
