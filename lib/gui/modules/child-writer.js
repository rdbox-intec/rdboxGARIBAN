    ...
    try {
      ...
      _.forEach(dests, (dest) => {
        _.forEach(dest.drive.mountpoints, (mountpoint) => {
          ...
          if (judgeObj.length) {
            log('Error device')
          } else if (mountpoint.label === 'HypriotOS') {
            log(`Mutch device: ${mountpoint.label}`)
            fs.writeFileSync(path.join(mountpoint.path, 'user-data'), options.userData.path)
          } else {
            log(`Not Mutch device: ${mountpoint.label}`)
          }
        })
      })
      terminate(exitCode)
