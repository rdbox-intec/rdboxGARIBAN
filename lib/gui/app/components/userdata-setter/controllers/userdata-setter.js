...
module.exports = function (
  ...
  this.generateRandomSSID = (type) => {
    if (type === 'be') {
      ...
      this.formDatas.backend.hostapd.ssid = `_${this.generateRandomString(12)}-${this.formDatas.clustername}`
      ...
      if (this.formDatas.apbg.hostapd.ssid !== '') {
        ...
        if (_.has(this.formDatas, 'apan.wpa.network')) {
  ...
  this.changeCtrlTimezone = () => {
    ...
    if (this.formDatas.backend.wpa.country === '') {
      this.formDatas.backend.wpa.country = this.formDatas.country
    }
    if (this.formDatas.apbg.wpa.country === '') {
      this.formDatas.apbg.wpa.country = this.formDatas.country
    }
    if (this.formDatas.apan.wpa.country === '') {
      this.formDatas.apan.wpa.country = this.formDatas.country
    }
    ...
    const pager = {
      ...
      pageUserdataEnvinfo: () => {
        try {
          ...
          const ntp = this.formDatas.ntp
          const tz = this.formDatas.timezone
          const lo = this.formDatas.locale
          analytics.logEvent('Result Timezone and Locale.', {
            ntp,
            tz,
            lo,
            applicationSessionUuid: store.getState().toJS().applicationSessionUuid,
