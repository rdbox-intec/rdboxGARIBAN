...
const app = angular.module('Gariban', [
...
app.run(($window) => {
  ...
  $window.addEventListener('beforeunload', (event) => {
    ...
    osDialog.showWarning({
      ...
      title: 'Are you sure you want to close Gariban?',
...
app.controller('HeaderController', function (OSOpenExternalService) {
   ...
   * in the archive `manifest.json`, or the default Gariban help page.
   *
   * @example
   * HeaderController.openHelpPage();
   */
  this.openHelpPage = () => {
    const DEFAULT_SUPPORT_URL = 'https://github.com/rdbox-intec/rdbox/wiki'
...
angular.element(document).ready(() => {
  settings.load().then(() => {
    angular.bootstrap(document, [ 'Gariban' ])
    userdatas.load()
  }).catch(exceptionReporter.report)
})
