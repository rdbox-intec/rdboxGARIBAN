...
module.exports = function (
  ...
  this.currentClusterName = 'rdbox'

  this.rdboxHardWare = -1
  ...
  this.reverseResolutionRdboxRole = [
    'master',
    'master',
    'slave',
    '',
  ...
  this.changeHostnameSuffix = () => {
    const hn = `${this.currentClusterName}-${this.reverseResolutionRdboxRole[this.rdboxRole]}-${this.hostnameSuffix}`
    if (_.isNil(this.hostnameSuffix)) {
      this.currentHostname = '?????'
    } else {
      if (this.rdboxRole === this.indexOfRdboxRole.BranchMaster && this.hostnameSuffix === '00') {
        this.hostnameSuffix = ''
        this.currentHostname = '?????'
        return
      }
      this.currentHostname = hn
      this.currentUserData.userDatas[this.currentUserData.currentCluster].cloudInit.hostname = hn
    }
  }
}
