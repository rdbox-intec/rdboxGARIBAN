...
ipc.connectTo(IPC_SERVER_ID, () => {
  ...
  ipc.of[IPC_SERVER_ID].on('write', async (options) => {
    ...
    try {
      ...
      await Bluebird.delay(DISCONNECT_DELAY)
      _.forEach(dests, (dest) => {
        const judgeObj = _.filter(results.errors, (errorObject) => {
          if (dest.device === errorObject.device) {
            return true
          }
          return false
        })
        ...
        try {
          const userDataYml = options.userData.currentRawText
          let alreadyNameFlg = false
          if (process.platform === 'darwin') {
            if (dest.drive.mountpoints) {
              _.forEach(dest.drive.mountpoints, (mountpoint) => {
                if (_.includes(mountpoint.path, 'HypriotOS')) {
                  fs.writeFileSync(path.join(mountpoint.path, 'user-data'), userDataYml)
                  alreadyNameFlg = true
                }
              })
              if (!alreadyNameFlg) {
                fs.writeFileSync(path.join('/Volumes', 'HypriotOS', 'user-data'), userDataYml)
              }
            } else {
              fs.writeFileSync(path.join('/Volumes', 'HypriotOS', 'user-data'), userDataYml)
            }
            ...
            if (dest.drive.mountpoints) {
              ...
              if (!alreadyNameFlg) {
                ...
                fs.writeFileSync(path.join('/mnt', user, 'HypriotOS', 'user-data'), userDataYml)
              }
            } else {
              const user = process.env.USER || ''
              fs.writeFileSync(path.join('/mnt', user, 'HypriotOS', 'user-data'), userDataYml)
            ...
            fs.writeFileSync(path.join(dest.drive.displayName, 'user-data'), userDataYml)
          } else {
            log(`INFO: Unsupported platform. ${process.platform}`)
          }
        } catch (error) {
          log(`INFO: ${error.message}. Continue.`)
        }
      })
      terminate(exitCode)
    } catch (error) {
