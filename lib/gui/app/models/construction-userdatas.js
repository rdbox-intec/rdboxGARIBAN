...
exports.getIndexOfWriteFs = (referrer, filesId) => {
  // eslint-disable-next-line lodash/matches-prop-shorthand
  const idx = _.findIndex(referrer.userDatas[referrer.currentCluster].rdbox.write_files, (write_file) => {
    return write_file.id === filesId
  })
  return idx
}
...
exports.setAuthorizedKeysOfRoot = (referrer, keysString) => {
  ...
  _.map(keysArray, (authKey) => {
    if (authKey.match(/^ssh-rsa/) &&
      // eslint-disable-next-line no-magic-numbers
      !_.includes(referrer.userDatas[referrer.currentCluster].cloudInit.users[0].ssh_authorized_keys, authKey)) {
      // eslint-disable-next-line no-magic-numbers
...
exports.addWpaSupplicantForYoursite = (referrer, country, ssid, passphrase = '', options = {}) => {
