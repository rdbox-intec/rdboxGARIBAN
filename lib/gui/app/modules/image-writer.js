 ...
 * })
 */
exports.performWrite = (image, drives, userdata, onProgress) => {
  // There might be multiple Gariban instances running at
  ...
  return new Bluebird((resolve, reject) => {
    ...
    const argv = _.attempt(() => {
      ...
      // This means we can't re-spawn Gariban as root from the same
      ...
      permissions.elevateCommand(argv, {
        ...
        if (!flashResults.cancelled && !_.get(flashResults, [ 'results', 'bytesWritten' ])) {
          throw errors.createUserError({
            ...
            description: 'Please try again, and contact the Gariban team if the problem persists',
            code: 'ECHILDDIED'
          })
        }
 ...
 * @param {String} image - image path
 * @param {Array<String>} drives - drives
 * @param {Object} config - config(user_data)
 * @returns {Promise}
 *
 * @example
 ...
 * })
 */
exports.flash = (image, drives, config) => {
  ...
  return exports.performWrite(image, drives, config, (state) => {
