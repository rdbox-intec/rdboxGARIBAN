...
exports.dumpYamlFromWriteFileList = (userdatas) => {
  ...
  _.forEach(userdatas.userDatas[userdatas.currentCluster].rdbox.write_files, (writeFileItem) => {
    const item = evalWriteFile[writeFileItem.type](writeFileItem)
    raw = `${raw}${item}\n`
  })
  console.log(raw)
}
...
${CLOUDINIT_HEDER_VIMMER}
`
  raw = `${raw}
${yaml.safeDump(userdatas.userDatas[userdatas.currentCluster].cloudInit)}
`
  this.dumpYamlFromWriteFileList(userdatas)
